...
function update_user_login_times() {
    ...
    $user->id = $USER->id;
    //$groups_members->id = $GROUPS_MEMBERS->id;
   ...
   $sql = "SELECT u.*, gm.*
           ...
           WHERE u.username=? AND u.id = gm.userid"; 
  
    $rs = $DB->get_recordset_sql($sql, $params);
            foreach ($rs as $u) {
                  //$USER->idnumber = $u->idnumber = $user->idnumber = $valueof;
                  $vari=$u->id;
                  ...
                  //$GROUPS_MEMBERS->component = $groups_members->component = $valueof;
                  //$DB->update_record_raw('groups_members', array('id'=> $vari,'groupid'=>$u->groupid,'userid'=>$u->userid,'timeadded'=>$u->timeadded,'component'=>$valueof,'itemid'=>$u->itemid));
                  $USER->theme = $user->theme = $u->theme = $ctheme;
                }
